/*
Problem Link: https://leetcode.com/problems/path-sum-iii/

Path Sum III

Given the root of a binary tree and an integer targetSum, return the number of paths where the sum of the values along the
path equals targetSum.
The path does not need to start or end at the root or a leaf, but it must go downwards (i.e., traveling only from parent nodes 
to child nodes).

Example 1:
Input: root = [10,5,-3,3,2,null,11,3,-2,null,1], targetSum = 8
Output: 3
Explanation: The paths that sum to 8 are shown.

Example 2:
Input: root = [5,4,8,11,null,13,4,7,2,null,null,5,1], targetSum = 22
Output: 3
 

Constraints:
The number of nodes in the tree is in the range [0, 1000].
-10^9 <= Node.val <= 10^9
-1000 <= targetSum <= 1000
 */


class Solution {
public:
    
    //Time Complexity: O(n)
    //Space Complexity: O(log n)
    void solver(TreeNode* root, int targetSum, int sum, int& cnt, unordered_map<int, int>& mp){
        
        if(root == NULL) return;
        
        sum += root->val;
        // cout << sum << endl;
        
        //using subarray sum equals k technique
        cnt += mp[sum - targetSum];
        mp[sum]++;
        
        solver(root->left, targetSum, sum, cnt, mp);
        solver(root->right, targetSum, sum, cnt, mp);
        
        mp[sum]--;
    }
    
    // No. of pathSum = targetSum
    int pathSum(TreeNode* root, int targetSum) {
        
        int num = 0, sum = 0; 
        unordered_map<int, int> mp;
        mp[0] = 1;
        solver(root, targetSum, sum, num, mp);
        
    return num;
    }
};



